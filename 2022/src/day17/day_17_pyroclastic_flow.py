...
class ShapeType(Enum):
    '''x,y coordinates for each five shape. 0,0 being bottom left corner'''
...
class Shape():
    """ Set of points that make up this shape.
    Has a factory methods to create Shape instances from Set of points or by ShapeType """
    ...
    def create_shape_by_type(cls, shape_type: str, origin: Point):
        """ Create an instance of shape offset by the supplied origin. """
        points = {Point(*p) + origin for p in ShapeType[shape_type].value}
        return cls(points)
...
class Tower():
    """ Fixed width tower that generates new shapes to drop, and moves shapes left and right as they drop """

    WIDTH = 7
    LEFT_WALL_X = 0
    RIGHT_WALL_X = 8    # right wall at x=8
    OFFSET_X = 3        # objects start with left edge at x=3
    OFFSET_Y = 4        # new rocks have a gap of 3 above top of highest settled rock
    FLOOR_Y = 0
    ...
    def __init__(self, jet_pattern: str) -> None:
        self._jet_pattern = itertools.cycle(enumerate(jet_pattern))  # continuously enumerate over gas movement pattern
        self._shape_generator = itertools.cycle(enumerate(item.name for item in ShapeType))  # continuously enumerate over shape types
        self.top = Tower.FLOOR_Y    # keep track the top of blocks
        ...
        self._cache: dict[tuple, tuple] = {}    # K=(shape_idx, jet_idx, rock_formation): V=(height, shape_ct)
    ...
    def _next_jet(self):
        """ Get the next left/right movement from the generator """
    ...
    def get_recent_formation(self) -> str:
        """ Convert last (top) 20 rows into a str representation. """
        rows = []
...
def part1(inputFile: str) -> int:
    ...
    tower = Tower(jet_pattern=pattern)
    for _ in range(2022):
        tower.drop_shape()
    return tower.top
...
def part2(inputFile: str) -> int:
    pattern = readInput(inputFile)
    tower = Tower(jet_pattern=pattern)  # Recreate the initial tower
